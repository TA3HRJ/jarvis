...
- [x] **Faz 6 — Uzaktan dispatch**:
  ...
  - [x] **iOS Shortcuts + Siri** — çalışıyor. Kurulum: Dictate Text → Get Contents of URL (GET), URL `http://100.100.59.67:8765/command?token=<kısa token>&text=<Dictated Text>`. **Kritik ayrıntı: iPhone'da da Tailscale kurulu ve aynı hesapla bağlı olmalı** (ilk denemede bu eksikti, istek hiç sunucuya düşmedi — telefon Tailscale'e bağlanınca çalıştı). Token JSON body/header yerine query param olarak taşınıyor — iOS Shortcuts'ta header/body key-value arayüzü telefon ekranında yazması zor olduğu için kasıtlı basitleştirme, güvenlik sınırı zaten Tailscale mesh'i. Canlı uçtan uca test edildi: telefon → Tailscale → sunucu (log'da telefonun Tailscale IP'sinden `200 OK` görüldü).
- [ ] **Faz 7 — Servisleştirme + sertleştirme** ← ŞU AN BURADAYIZ: systemd servis (API için zaten yapıldı, Faz 6'ya bak — diğer bileşenler kalıyor), crash recovery, log/observability, yetki allowlist, denetim logu
