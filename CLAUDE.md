...
- [x] **Faz 7 — Servisleştirme + sertleştirme** (yetki allowlist kasıtlı olarak ertelendi, gerçek bir tehdit ortaya çıkınca eklenecek):
  ...
  - [x] **Gerçek eylemler bağlandı** (`src/jarvis/actions.py`): Katman 1-3 sadece niyet sınıflandırıyordu, hiçbir eylemi gerçekten yapmıyordu — ham "Niyet: saat_kac..." metnini sesli okuyordu (canlı testte fark edildi ve düzeltildi). Artık `saat_kac`, `ekrani_kilitle`, `sesi_kis`, `sesi_ac`, `ses_ayarla` gerçekten çalışıyor (loginctl/wpctl); `hava_durumu`/`alarm_kur` henüz uygulanmadığını dürüstçe söylüyor (sahte "yapıldı" cevabı yok).
  - [x] **`jarvis-main.service`** systemd user servisi (`~/.config/systemd/user/`, `EnvironmentFile=.env`, `Restart=on-failure`) — `jarvis-api.service` ile aynı desen. İkisi de canlı sesle uçtan uca test edildi: "Hey Jarvis" algılandı (skor ~0.9), "saat kaç" doğru transkribe edildi, gerçek saat sesli söylendi; "sesi kıs" gerçekten ses seviyesini değiştirdi (doğrulandı: `wpctl get-volume`).
  - [x] **Canlı sesli kullanımda bulunan 3 hata düzeltildi:**
    - Wake word'ün ("Hey Jarvis") sesi komut kaydına karışıyordu ("H.A.R.V.I.S Sesi Kıs" gibi yanlış transkripsiyona yol açıyordu, yönlendirici eşleştiremeyip Katman 4'e düşüyordu) — wake word algılandıktan sonra `POST_WAKE_DISCARD_SECONDS` (0.4s) kısa bir atlama eklendi (`main.py`).
    - Piper TTS, DeepSeek'in ürettiği markdown'ı (`**kalın**`) harfiyen okuyordu ("yıldız yıldız") — `tts.py`'ye savunma amaçlı markdown temizleme eklendi (LLM'e "markdown kullanma" demek yetmiyor, koddan da garantile).
    - **CUDA "out of memory" çökmesi** (systemd otomatik kurtardı, ama neden araştırıldı): Whisper (~3.3GB) ve Katman 3'ün yerel modeli (~2.5GB) aynı süreçte/anda GPU'da kalırsa 6GB'ı aşıyordu — CLAUDE.md'nin "lazy load + idle unload" kuralı Faz 4'te yazılmış ama hiç uygulanmamıştı. `layer3.py`'ye kullanım sonrası otomatik GPU boşaltma eklendi, doğrulandı (kullanım sonrası GPU belleği ~180MB'a düştü, gigabaytlık kalıntı yok).
  - [x] **Tek süreç birleştirmesi**: `jarvis-api.service` kaldırıldı (disable edildi), API artık `jarvis-main.service` içinde arka plan thread'i olarak çalışıyor (`main.py`'de `_run_api_server`) — router/brain modelleri artık tek kopya, iki ayrı süreç GPU belleğini paylaşmıyor. CLAUDE.md kural #2 artık tam uygulanıyor. Doğrulandı: tek `jarvis-main.service` altında hem sesli döngü hem `/command` API'si çalışıyor.
  - [x] **Yapılandırılmış log + denetim izi** (`logsetup.py`, `dispatch.py`): her komut `kaynak=local|remote metin=... niyet=... katman=... -> yanıt` formatında INFO seviyesinde loglanıyor, journald üzerinden kalıcı/aranabilir (`journalctl --user -u jarvis-main.service`). Doğrulandı.
  - [x] **Crash recovery**: `Restart=on-failure` zaten oradaydı, gerçek bir CUDA OOM çökmesinde otomatik yeniden başlattığı canlı olarak gözlemlendi (yukarıdaki hata düzeltmeleri bölümüne bak).
  - [~] **Yetki allowlist**: ayrı bir özellik olarak eklenmedi — bilinçli karar. Güvenlik sınırı zaten üç katmanlı: Tailscale-only ağ erişimi + token doğrulama + Katman 4'ün kod çalıştırma aracı zaten bwrap sandbox'ında (ağsız, salt-okunur kök). Şu anki eylemlerin (`actions.py`) hiçbiri yıkıcı değil (ekran kilitleme, ses seviyesi). Kataloğa daha riskli eylemler eklenirse (dosya silme, uygulama kapatma vb.) ya da Katman 4'e gerçek masaüstü kontrolü verilirse o zaman gerekli — şimdiden var olmayan bir tehdide karşı inşa etmemek tercih edildi.
