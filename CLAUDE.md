...
- [~] **Faz 7 — Servisleştirme + sertleştirme** ← ŞU AN BURADAYIZ:
  ...
  - [ ] Log/observability (yapılandırılmış log, hata izleme), yetki allowlist (özellikle Katman 4'ün `run_sandboxed_command`/masaüstü erişimi için), denetim logu — henüz yapılmadı.
  - [ ] **Açık mimari sorun (henüz çözülmedi):** `jarvis-api.service` ve `jarvis-main.service` ayrı süreçler, ikisi de `router`/`brain` modüllerini bağımsız import ediyor — CLAUDE.md kural #2 ("tek süreç orkestratör") ihlali. Şimdilik idle-unload OOM riskini azalttı ama asıl çözüm ikisini tek sürece birleştirmek (API'yi orkestratörün içinde bir arka plan thread'i yapmak).
