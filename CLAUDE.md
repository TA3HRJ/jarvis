...
- [~] **Faz 5 — Beyin** ← ŞU AN BURADAYIZ (kısmen tamamlandı, API anahtarı ile canlı test bekleniyor):
  ...
  - [x] **Bulut LLM tool-calling döngüsü** (`src/jarvis/brain.py`): `claude-opus-5`, Anthropic Tool Runner, adaptive thinking, `run_sandboxed_command`/`remember`/`recall` araçları bağlandı. **Kod yazıldı ama `ANTHROPIC_API_KEY` olmadığı için canlı test edilemedi.**
