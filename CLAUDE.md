...
- [~] **Faz 6 — Uzaktan dispatch** ← ŞU AN BURADAYIZ (kısmen tamamlandı, kullanıcı tarafı adımlar bekleniyor):
  ...
  - [ ] **Tailscale** — kurulum + hesap girişi sudo + tarayıcı gerektiriyor, kullanıcı kendi terminalinde yapmalı: `sudo pacman -S tailscale && sudo systemctl enable --now tailscaled && sudo tailscale up`. Kurulunca API sunucusu `127.0.0.1` yerine tailscale arayüzüne (`tailscale ip -4`) bind edilecek.
  - [ ] **iOS Shortcuts + Siri** — telefon tarafında manuel kurulum: Kısayollar uygulamasında "Get Contents of URL" eylemi, POST, URL `http://<tailscale-ip>:8765/command`, header `Authorization: Bearer <JARVIS_API_TOKEN>`, body `{"text": "[Konuşulan Metin]"}` (Dictate Text eylemiyle birleştirilip Siri'ye "Jarvis'e sor" gibi bir isimle atanabilir). Tailscale telefonda da kurulu ve aynı mesh'te olmalı.
